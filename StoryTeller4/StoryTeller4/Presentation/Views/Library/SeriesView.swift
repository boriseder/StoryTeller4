...
struct SeriesView: View {
    ...
    var body: some View {
        ...
        .refreshable { await viewModel.loadSeries() }
        .toolbar {
            ToolbarItem(placement: .navigationBarTrailing) {
                HStack(spacing: DSLayout.contentGap) {
                    if !viewModel.series.isEmpty { sortMenu }
                    SettingsButton()
                }
            }
        }
        .task { await viewModel.loadSeriesIfNeeded() }
    }
        ...
        ZStack {
            ScrollView {
                LazyVStack(spacing: DSLayout.contentGap) {
                    // Guard stellt sicher dass wir nur rendern wenn API konfiguriert ist.
                    // SeriesView ist in der Navigation nur nach Login erreichbar.
                        ...
                        ForEach(viewModel.filteredAndSortedSeries) { series in
                            SeriesSectionView(
                                ...
                                onBookSelected: { book in
                                    Task {
                                        await viewModel.playBook(book, restoreState: true)
                                    }
                                }
                            )
                        }
