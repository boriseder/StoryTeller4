...
struct MediaProgress: Codable, Identifiable, Sendable {
    ...
    nonisolated func encode(to encoder: Encoder) throws {
        ...
        if let finishedAt { try container.encode(TimestampConverter.serverTimestamp(from: finishedAt), forKey: .finishedAt) }
    }

    // Note: chapterIndex(at:) on Book may be @MainActor — callers that need
    // this inside an actor must hop via MainActor.run. See PlaybackRepository.
    func chapterIndex(for book: Book) -> Int { book.chapterIndex(at: currentTime) }
...
// MARK: - PlaybackState
//
// All Codable methods and mutating helpers are explicitly nonisolated so
// they can be called from actor-isolated contexts (PlaybackRepository) and
// from @MainActor contexts (ViewModels) without isolation conflicts.
...
struct PlaybackState: Sendable {
    ...
    // nonisolated: called from nonisolated contexts including updating() and
    // PlaybackRepository (actor). Without this, Swift infers @MainActor on
    // the memberwise init whenever any stored property type is @MainActor-bound.
    nonisolated init(
        libraryItemId: String,
        currentTime: Double,
        duration: Double,
    ...
    // nonisolated: called from PlaybackRepository (actor context).
    nonisolated init(from mediaProgress: MediaProgress, chapterIndex: Int = 0) {
...
// MARK: - PlaybackState Codable (nonisolated throughout)

extension PlaybackState: Codable {
    enum CodingKeys: String, CodingKey {
        case libraryItemId, currentTime, duration, isFinished
        case lastUpdate, chapterIndex, needsSync
    }
    ...
    nonisolated func encode(to encoder: Encoder) throws {
        ...
        try c.encode(needsSync,     forKey: .needsSync)
    }
}
