...
enum ValidationResult: Sendable {
    ...
    var missingFiles: String { failureReason ?? "Unknown error" }
}
...
final class DefaultDownloadValidationService: DownloadValidationService {
    ...
    func validateBookIntegrity(bookId: String, storageService: DownloadStorageService) -> ValidationResult {
        ...
        guard fileManager.fileExists(atPath: metadataFile.path) else { return .invalid(reason: "Metadata file missing") }
        guard let data = try? Data(contentsOf: metadataFile), (try? JSONDecoder().decode(Book.self, from: data)) != nil else { return .invalid(reason: "Invalid metadata file") }
        guard let audioInfo = storageService.loadAudioInfo(for: bookId) else { return .invalid(reason: "Audio info missing") }
        ...
        for index in 0..<audioInfo.audioTrackCount {
            ...
            if !fileManager.fileExists(atPath: audioFile.path) { return .invalid(reason: "Missing audio track \(index + 1)") }
            if !validateFile(at: audioFile, minimumSize: minimumAudioSize) { return .invalid(reason: "Audio track \(index + 1) is corrupted") }
        }
        return .valid
    }
    ...
    func validateFile(at url: URL, minimumSize: Int64) -> Bool {
        ...
        if let attr = try? fileManager.attributesOfItem(atPath: url.path), let size = attr[.size] as? Int64 { return size >= minimumSize }
        return false
    }
}
